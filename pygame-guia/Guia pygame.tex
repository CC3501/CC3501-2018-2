...
\def\tituloauxiliar {Pygame}
\def\temaatratar {Autor: Pablo Pizarro, \href{http://ppizarror.com}{@ppizarror.com}}

\def\autordeldocumento {Pablo Pizarro}
\def\nombredelcurso {Modelación y Computación Gráfica para Ingenieros}
\def\codigodelcurso {CC3501}
...
\newp El esquema de funcionamiento de cualquier aplicación en Python es típicamente \footnote{En el sentido que, a pesar que pueden haber variaciones, a\textit{Grosso modo} presenta el esquema presentado.} el siguiente:
\lstset{style=Python}
...
# Se cargan fuentes, sonidos e imágenes
sonido_muere = pygame.mixer.Sound('sounds/dead.wav')  # Sonidos
fuente_juego = pygame.font.Font('font/8bit.ttf', 40)  # Fuentes
imagen_mono = pygame.image.load('images/imagen1.png') # Imágenes
icono_juego = pygame.image.load('images/icon.png')

...

# Se crea la pantalla
surface = pygame.display.set_mode((640, 480)) # Crea una ventana de 640 por 480 pixeles, retorna el CANVAS para dibujar en pantalla
pygame.display.set_caption('Titulo del juego') # Título de la ventana
pygame.display.set_icon(icono_juego) # Ícono de la ventana
...
while True:
	...
	# Ve teclas pulsadas e interactúa con los modelos
	keys = pygame.key.get_pressed()
	if keys[K_w]:
		jugador1.subir()
		...
	
	# Los modelos interactúan entre sí, etc
	jugador1.colisiona(jugador2)
	...
	
	# Se pinta el fondo
	surface.fill(COLOR_BLACK) # También se puede dibujar una imagen
	...
	# Vuelca todo lo dibujado en pantalla
	pygame.display.flip()

\end{lstlisting}

\newpage
\subsectionanum{Imágenes}

Las imágenes (archivos externos como \textbf{png}, \textbf{gif}) se cargan utilizando la instrucción \textit{pygame.image.load(dir)}. Esta retorna un objeto del tipo \textbf{surface} (el cual conceptualmente es como un canvas). 

\newp Para dibujar todo lo que es imagen en pantalla se debe volcar el \textit{surface} generado en el \textbf{surface} de la pantalla (es como pegar un canvas pequeño en uno más grande), esto se hace con la función \textbf{blit}, la cual toma por parámetro el surface de la imagen cargada y una posición \texttt{(x,y)} tal como se muestra a continuación:
\lstset{style=Python}
\begin{lstlisting}[language=Python, caption=Pegar imágenes en la pantalla.]
imagen_mono = pygame.image.load('images/imagen1.png')
surface.blit(imagen_mono, (30, 60)) # Recordar que surface era el objeto que retornaba al crear la ventana, ver línea 20 del código fuente 1
\end{lstlisting}

En este caso se dibuja la imagen cargada en pantalla en las coordenadas \texttt{30, 60} (30 píxeles horizontales y 60 píxeles verticales, considerando el \texttt{(0,0)} como la esquina superior izquierda). \\

Para ver la lista completa de funciones revisa este enlace: \url{https://www.pygame.org/docs/ref/image.html}

\subsectionanum{Líneas, polígonos, etc}

Pygame ofrece la sub-librería \textbf{pygame.draw} para realizar una variedad de acciones. A continuación se presenta una lista de funciones más comunes:

\begin{enumerate}
	\item Dibujar un polígono: \textit{pygame.draw.polygon(surface, color, pointlist, width=0)}. \\\textbf{Pointlist} es la lista de puntos del polígono, por ejemplo: \texttt{[(10, 40), (60, 30), (40, 60) ..]}, \textbf{color} es el color del polínono y \textbf{width} es el ancho del borde del polígono, si \textit{width=0} entonces el polígono tiene un relleno del color deseado.
	\item Dibujar una línea: \textit{pygame.draw.line(surface, color, [x1, y1],
		[x2, y2], width)}.\\En este caso se dibuja una línea recta entre \texttt{(x1, y1)} y \texttt{(x2, y2)} de un color \textbf{color} y un ancho \textbf{width} (en pixeles).
	
\end{enumerate}

Para ver la lista completa de funciones revisa este enlace: \url{https://www.pygame.org/docs/ref/draw.html} \\


\textbf{NOTA}: Los colores están definidos por una tupla de 3 números de la forma \texttt{(R, G, B)} en donde \texttt{R} indica el valor de la componente roja (0 no hay rojo, 255 es el máximo valor), \texttt{G} es la componente verde y \texttt{B} es la componente azul. Ejemplos:
\begin{lstlisting}[language=Python, caption=Colores en Pygame.]
COLOR_BLACK = (0, 0, 0)
...
\subsectionanum{Sonidos}

Los sonidos de cargan con la instrucción \textit{pygame.mixer.Sound(dir)}, en donde se retorna un objeto del tipo Sound. Cada objeto posee varias funciones, entre las cuales se encuentran:
\begin{lstlisting}[language=Python, caption=Sonidos en Pygame.]
sonido_muere = pygame.mixer.Sound('sounds/dead.wav')
sonido_muere.play(0)  # Sonido se reproduce 1 vez
sonido_muere.play(1)  # Sonido se reproduce 2 veces
sonido_muere.play(n)  # Sonido se reproduce n+1 veces
sonido_muere.play(-1) # Sonido se reproduce infinitas veces
sonido_muere.stop()   # Sonido se detiene
sonido_muere.pause()  # Sonido se pausa
\end{lstlisting}

\newp Para ver la lista completa de funciones revisa este enlace: \url{https://www.pygame.org/docs/ref/mixer.html}

\subsectionanum{Fuentes}

Para cargar una fuente se ejecuta la siguiente instrucción: \textit{pygame.font.Font(dir, size)} en donde se carga la fuente en la ubicación \textbf{dir} con un tamaño de letra de \textbf{size} píxeles. Para crear un texto se debe generar su \textit{surface} asociado, esto es, se debe renderizar una imagen de un texto con esa fuente para luego volcar ese \textit{surface} con la función \textbf{blit} (ver subcapítulo de imágenes).

\newp Para renderizar un texto se usa la función \textit{font.render(text, antialias, color)} en donde \textbf{font} es la fuente cargada, \textbf{text} es el texto a renderizar, \textbf{antialias} indica si el texto a renderizar tiene \textit{antialiasing} o no (por lo general se deja como 1) y \textbf{color} es el color del texto. Esta función retorna un \textit{surface} el cual posteriormente se vuelca en pantalla con la función \textbf{blit}.

\newp A continuación se muestra un ejemplo que escribe \texttt{'HOLA MUNDO'} en el \textbf{surface} de la ventana en la posición \texttt{(100, 500)}:
\begin{lstlisting}[language=Python, caption=Fuentes con Python.]
# Se carga la fuente
fuente_juego = pygame.font.Font('font/8bit.ttf', 40)

# Se dibuja un texto
text = font.render('HOLA MUNDO', 1, COLOR_WHITE)

# Se vuelca el texto en pantalla
surface.blit(text, (100, 500))
\end{lstlisting}

\newp Para ver la lista completa de funciones revisa este enlace: \url{https://www.pygame.org/docs/ref/font.html}
